...
| --- | --- | --- |
| Ловля токенов, запуск, группы, профили, инфо об игроке | да | да |
| Лаунчер | обычный `.exe`, JAR, новый `.exe`, свой | JAR, свой |
| Импорт по логину/паролю, обновление токенов | да | нет |
| Автозапуск клиента (клик «ИГРАТЬ») | да | нет |
...
Автообновление сверяет имя ассета с вариантом, которым собран сам бинарь, поэтому имена важны:

| Файл | Вариант | Требует |
| --- | --- | --- |
| `AccountChanger.exe` | Windows | — |
| `AccountChanger-linux-x86_64` | нативное окно | `libwebkit2gtk-4.1.so.0`, glibc ≥ 2.34 |
| `AccountChanger-linux-x86_64-webkit40` | нативное окно | `libwebkit2gtk-4.0.so.37`, glibc ≥ 2.34 |
| `AccountChanger-linux-x86_64-nogui` | UI в браузере | ничего, статический |

Варианты **не взаимозаменяемы**: сборка под 4.1 не запустится там, где только 4.0. Апдейтер поэтому берёт строго свой ассет, а если его в релизе нет - просто не предлагает обновление (лучше без обновления, чем незапускающийся бинарь).

> **Грабли сборки:** Go 1.25 не учитывает build-тег в ключе кэша для `#cgo <tag> pkg-config:`. Если собирать варианты с одним `GOCACHE`, второй молча возьмёт pkg-config-результат первого — `./build.sh webkit40` после `./build.sh` слинкуется с 4.1. `build.sh` держит отдельный кэш на вариант; если собираешь руками — делай так же.